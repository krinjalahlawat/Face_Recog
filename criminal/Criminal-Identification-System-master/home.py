...
def basicPageSetup(pageNo):
    ...
    img1 = Image.open("images/th2.png")
    ...
    heading = tk.Label(pages[pageNo], fg="#6FBEBA", bg="black", font="Arial 20 bold", pady=10)
    heading.pack()
    ...
    right_frame = tk.LabelFrame(content, text="Detected Criminals", bg="#6FBEBA", font="Arial 20 bold", bd=4,
                             foreground="black", labelanchor="n")
...
def showImage(frame, img_size):
    ...
    if (img_label == None):
        img_label = tk.Label(left_frame, image=img, bg="#6FBEBA")
        img_label.image = img
...
def selectMultiImage(opt_menu, menu_var):
    ...
    if(len(path_list) < 5):
        ...
        prev_slide = tk.Button(slide_control_panel, image=back_img, bg="", bd=0, highlightthickness=0,
        ...
        slide_caption = tk.Label(slide_control_panel, text="Image 1 of {}".format(len(img_list)), fg="#ff9800",
                              bg="#6FBEBA", font="Arial 15 bold")
        slide_caption.grid(row=0, column=1)

        next_slide = tk.Button(slide_control_panel, image=next_img, bg="#6FBEBA", bd=0, highlightthickness=0,
                            activebackground="#6FBEBA", command=lambda : getNewSlide("next"))
...
def register(entries, required, menu_var):
    ...
    path = os.path.join(current_path,'face_samples', "temp_criminal")
...
def getPage1():
    ...
    btn_grid = tk.Frame(left_frame, bg="#6FBEBA")
    btn_grid.pack()

    tk.Button(btn_grid, text="Select Images", command=lambda: selectMultiImage(opt_menu, menu_var), font="Arial 15 bold", bg="#6FBEBA",
           fg="black", pady=10, bd=0, highlightthickness=0, activebackground="#6FBEBA",
           activeforeground="white").grid(row=0, column=0, padx=25, pady=25)


    # Creating Scrollable Frame
    canvas = tk.Canvas(right_frame, bg="#B2C6E4", highlightthickness=0)
    ...
    for i, field in enumerate(input_fields):
        row = tk.Frame(scroll_frame, bg="#6FBEBA")
        row.pack(side="top", fill="x", pady=15)

        label = tk.Text(row, width=20, height=1, bg="#6FBEBA", fg="black", font="Arial 13", highlightthickness=0, bd=0)
        ...
        if(i != ip_len-1):
            ...
            opt_menu.configure(font="Arial 13", bg="light yellow", fg="black", bd=0, highlightthickness=0, activebackground="#90ceff")
    ...
    tk.Button(scroll_frame, text="Register", command=lambda: register(entries, required, menu_var), font="Arial 15 bold",
           bg="light yellow", fg="black", pady=10, padx=30, bd=0, highlightthickness=0, activebackground="#091428",
...
def getPage2():
    ...
    tk.Button(btn_grid, text="Select Image", command=selectImage, font="Arial 15 bold", padx=20, bg="black",
            fg="#6FBEBA", pady=10, bd=4, highlightthickness=0, activebackground="#6FBEBA",
            activeforeground="white").grid(row=0, column=0, padx=25, pady=25)
    tk.Button(btn_grid, text="Recognize", command=startRecognition, font="Arial 15 bold", padx=20, bg="black",
           fg="#6FBEBA", pady=10, bd=4, highlightthickness=0, activebackground="#6FBEBA",
...
img = Image.open("images/top9.png")
img=img.resize((1290,170))
logo= ImageTk.PhotoImage(img)


tk.Label(pages[0], image=logo, bg="black").pack()

btn_frame = tk.Frame(pages[0], bg="black", pady=50)
btn_frame.pack()

aimg=tk.Button(btn_frame, text="Register new Criminal", command=getPage1)
tk.Button(btn_frame, text="Recognize Criminal", command=getPage2)
tk.Button(btn_frame, text="Video Inspection", command=getPage3)


for btn in btn_frame.winfo_children():
    btn.configure(font="Arial 20", width=17, bg="#6FBEBA", fg="black",
        pady=8, bd=2, highlightthickness=4, activebackground="white", activeforeground="black")
    btn.pack(pady=16)
